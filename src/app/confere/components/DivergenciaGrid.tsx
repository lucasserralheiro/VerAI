...
function Linhas({ linhas }: { linhas: LinhaDoGrid[] }) {
	return (
		<>
			{linhas.map((linha) => (
				<tr key={`${linha.codigo}-${linha.descricao}`} className="border-b border-confere-line last:border-0">
					...
					<td
						className={`relative whitespace-nowrap px-3 py-1.5 text-right font-semibold tabular-nums ${
							ehNegativo(linha.saldo) ? "bg-red-50 text-red-700" : "text-confere-navy-600"
						}`}
					>
						{linha.saldo}
...
function Legenda({
	...
	return (
		<dl className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-confere-navy-300">
			{temPerfil && (
	...
	return (
		<div className="mb-3">
			{/* `R-PAN-08` — com o painel de análise acima, dois títulos "Divergências"
			    ...
			    o título diz isso. */}
			<h2 className="text-lg font-semibold text-confere-navy-600">
	...
	if (divergencias.length === 0) {
		return (
			<div className="mt-6">
				...
				<p className="mt-2 rounded-md border border-confere-teal-100 bg-confere-teal-50 p-4 text-sm text-confere-teal-700">
