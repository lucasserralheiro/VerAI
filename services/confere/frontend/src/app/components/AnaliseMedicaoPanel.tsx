...
const TARJA: Record<Classificacao, string> = {
	CRITICO: "border-l-severidade-critico",
	MAIOR_RELEVANCIA: "border-l-severidade-maior",
	DIVERGENTE: "border-l-navy-600",
	SEM_DIVERGENCIA: "border-l-brand-green",
};

const SELO: Record<Classificacao, string> = {
	CRITICO: "bg-severidade-critico text-white",
	MAIOR_RELEVANCIA: "bg-severidade-maior-fundo text-severidade-maior",
	DIVERGENTE: "bg-navy-50 text-navy-600",
	SEM_DIVERGENCIA: "bg-severidade-conforme-fundo text-brand-green-ink",
};
...
function Marca({ texto, tom }: { texto: string; tom: "perfil" | "critica" }) {
	const estilo =
		tom === "perfil"
			...
			: "bg-severidade-critico text-white";
	return (
								...
								<td
									className={`relative whitespace-nowrap px-3 py-1.5 text-right font-semibold tabular-nums ${
										ehNegativo(linha.saldo)
											? "bg-severidade-critico-fundo text-severidade-critico"
											: "text-navy-600"
									}`}
								>
									{linha.saldo}
...
function Bloco({ situacao }: { situacao: SituacaoDaAnalise }) {
	...
	return (
		<details
			className={`overflow-hidden rounded-md border border-l-4 border-line bg-white ${TARJA[situacao.classificacao]}`}
		>
			<summary className="flex cursor-pointer list-none items-center gap-2.5 px-4 py-3 hover:bg-neutral-50 [&::-webkit-details-marker]:hidden">
				<svg
					...
					strokeWidth={2}
					aria-hidden="true"
					className="h-3 w-3 shrink-0 text-navy-300 transition-transform motion-reduce:transition-none [details[open]_&]:rotate-90"
				>
					<path d="m4 2 4 4-4 4" strokeLinecap="round" strokeLinejoin="round" />
				</svg>
				<h3 id={id} className="text-sm font-semibold text-navy-800">
					{situacao.rotulo}
				</h3>
				<span className="text-xs text-navy-300">— {situacao.glosa}</span>
				<span
			...
			{/* `R-PAN-04` — situação sem itens aparece assim mesmo, e com a contagem
			    ...
			    quem abrir. */}
			{vazia ? (
				<p className="px-4 pb-3.5 text-sm text-brand-green-ink">
				...
				<>
					...
					{situacao.classificacao === "SEM_DIVERGENCIA" && situacao.perfis_ou_pacotes > 0 && (
						<p className="px-4 pb-3 text-xs text-navy-300">
...
export function AnaliseMedicaoPanel({ analise }: { analise: Analise }) {
	return (
		<section className="mt-7" aria-labelledby="titulo-analise">
			<h2 id="titulo-analise" className="text-lg font-semibold text-navy-600">
				Análise da medição
			</h2>
			<p className="mb-4 flex flex-wrap gap-x-2 gap-y-0.5 text-sm text-navy-300">
				<span>
					Contrato{" "}
				...
				<span>
					<strong className="font-semibold tabular-nums text-navy-600">
						{analise.total_itens}
					</strong>{" "}
					itens analisados
				</span>
