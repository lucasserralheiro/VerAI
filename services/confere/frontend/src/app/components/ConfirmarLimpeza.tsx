...
export function ConfirmarLimpeza({ aberto, haRelatorio, onConfirmar, onCancelar }: Props) {
	...
	return (
		<dialog
			...
			className="w-[min(28rem,calc(100vw-2rem))] rounded-lg border border-line bg-white p-6 shadow-lg"
		>
			<h2 id="confirmar-limpeza-titulo" className="text-lg font-semibold text-navy-600">
				Limpar e recomeçar?
			</h2>
			...
			<form method="dialog" className="mt-6 flex flex-wrap justify-end gap-3">
				<button
					ref={cancelar}
					value="cancelar"
					className="rounded-md border border-line bg-white px-5 py-2.5 text-sm font-semibold text-navy-600 transition hover:bg-navy-50"
				>
					Cancelar
				</button>
				{/* `severidade-critico` é o vermelho já medido do projeto — 6,44:1
				    contra branco (tailwind.config.ts). O hover usa o passo mais
				    escuro da paleta padrão, que a escala semântica não tem. */}
				<button
					value="limpar"
					className="rounded-md bg-severidade-critico px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-red-800"
				>
					Limpar tudo
				</button>
			</form>
