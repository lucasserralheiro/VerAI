...
interface Props {
	...
	podeLimpar: boolean;
	onLimpar: () => void;
	refLimpar: React.RefObject<HTMLButtonElement>;
	refPrimeiroCampo: React.RefObject<HTMLInputElement>;
	...
	refAditivos: React.RefObject<HTMLInputElement>;
...
export function UploadForm({
	...
	return (
		<form onSubmit={submeter} className="rounded-lg border border-line bg-white p-6 shadow-sm">
				...
				{CAMPOS.map((campo, indice) => {
					...
					return (
						...
						<label
							...
							className="flex cursor-pointer flex-col gap-2 rounded-md border border-teal-100 bg-teal-50/40 p-4 transition hover:border-teal-400 has-[:focus-visible]:border-teal-400 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-teal-500 has-[:focus-visible]:ring-offset-2"
						>
							<span className="text-sm font-semibold text-navy-600">
								{campo.rotulo}
							</span>
							<span id={`${campo.nome}-descricao`} className="text-xs text-navy-300">
								{campo.descricao}
							</span>
							<input
								type="file"
			...
			<label
				...
				className="mt-4 flex cursor-pointer flex-col gap-2 rounded-md border border-line bg-white p-4 transition hover:border-teal-400 has-[:focus-visible]:border-teal-400 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-teal-500 has-[:focus-visible]:ring-offset-2"
			>
				<span className="text-sm font-semibold text-navy-600">
					{CAMPO_ADITIVOS.rotulo}
				</span>
				<span id={`${CAMPO_ADITIVOS.nome}-descricao`} className="text-xs text-navy-300">
			...
			<button
				...
				className={`mt-6 inline-flex items-center gap-2 rounded-md px-5 py-2.5 text-sm font-semibold transition ${
					bloqueado
						? "cursor-not-allowed bg-navy-100 text-navy-600"
			...
			{podeLimpar && (
				<button
					...
					ref={refLimpar}
					onClick={onLimpar}
					className="ml-3 mt-6 inline-flex items-center rounded-md border border-line bg-white px-5 py-2.5 text-sm font-semibold text-navy-600 transition hover:bg-navy-50"
				>
					Limpar
				</button>
			)}
			...
			{processando && (
				<p className="mt-3 text-xs text-navy-300">
					Extraindo o contrato, reconciliando e montando os anexos. Pode levar até um
					minuto.
				</p>
			)}

			{!completo && (
				<p id="upload-pendente" className="mt-3 text-xs text-navy-300">
