...
import { AnaliseMedicaoPanel } from "./AnaliseMedicaoPanel";
...
/** `dd/mm/aaaa` vira `mm/aaaa`: no cabeçalho do resultado o dia não acrescenta
 *  nada — a medição é da competência, não do dia. */
...
function Conteudo({
	...
	return (
		<section className="mt-6">
			<div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-confere-teal-100 bg-confere-teal-50 p-5">
				<div>
					<h2
						ref={titulo}
						tabIndex={-1}
						className="text-sm font-semibold text-confere-teal-700 outline-none"
					>
						Relatório gerado
					</h2>
					<p className="mt-1 text-sm text-confere-navy-600">
						<strong>{relatorio.total_divergencias}</strong> de {relatorio.total_linhas} itens
						com divergência entre contratado e medido
					</p>
					{/* Referência do contrato e competência: era o slot de contexto da
					    barra de aplicação (ESPEC 007 `R-CAB-05`/`R-CAB-06`), removida
					    junto com a faixa de marca. O dado continua valendo — quem
					    confere precisa saber *de que* contrato é o relatório que acabou
					    de baixar — e aqui ele nasce e é lido no mesmo lugar, em vez de
					    viajar pro topo da tela. O dia some: a medição é da
					    competência. */}
					<p className="mt-1 text-xs tabular-nums text-confere-navy-600">
						<span className="font-mono font-semibold text-confere-brand-navy">
							{relatorio.contrato_referencia}
						</span>
						{competencia(relatorio.data_levantamento) && (
							<>
								<span aria-hidden="true" className="mx-2 text-confere-navy-100">
									·
								</span>
								<span>{competencia(relatorio.data_levantamento)}</span>
							</>
						)}
					</p>
				</div>
				<div className="flex flex-wrap gap-2.5">
					<a
						...
						className="rounded-md bg-confere-teal-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-confere-teal-600"
					>
						Baixar DOCX
					</a>
					...
					<a
						...
						className="rounded-md border border-confere-teal-300 bg-white px-5 py-2.5 text-sm font-semibold text-confere-teal-700 transition hover:bg-confere-teal-50"
					>
						Baixar análise (XLSX)
					</a>
				</div>
			...
			{relatorio.avisos.length > 0 && (
				<section className="mt-6" aria-labelledby="titulo-avisos">
					<h2 id="titulo-avisos" className="text-lg font-semibold text-confere-navy-600">
					...
					{/* `R-AVI-05` — a frase diz o que o bloco **não** é antes de dizer o
					    ...
					    que trava (`D-06`). */}
					<p className="mb-3 mt-1 text-sm text-confere-navy-600">
