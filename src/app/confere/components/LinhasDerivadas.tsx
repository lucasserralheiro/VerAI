		...
		<>
			{linhas.map((linha) => (
				<tr key={`${linha.linha}-${linha.codigo}`} className="border-b border-confere-line last:border-0">
					...
					<td className="whitespace-nowrap bg-amber-50 px-3 py-1.5 text-center font-semibold tabular-nums text-confere-navy-800">
						{linha.saiu}
					</td>
				</tr>
			))}
...
export function LinhasDerivadas({ linhas }: { linhas: LinhaDerivada[] }) {
	...
	return (
		<section className="mt-6" aria-labelledby="titulo-derivadas">
			...
			<div className="mt-3 overflow-hidden rounded-md border border-confere-line bg-white">
				<div
