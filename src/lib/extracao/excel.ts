...
/**
 * Candidatos a conferência de uma planilha — TODA célula numérica de toda
 * linha com dado, não só linha com rótulo de total. Pedido explícito:
 * restringir a "total"/"subtotal" deixava de fora item de preço comum
 * ("Storage Premium", 11200) — o mesmo raciocínio já aplicado a
 * PDF/Word em `conferirTotais.ts`. Rótulo é a primeira célula de TEXTO da
 * própria linha (o "nome" da linha, tipo a descrição do item); sem
 * nenhuma célula de texto, cai pro cabeçalho da coluna. A MESMA linha
 * lida por `processarPlanilha`, então uma célula de fórmula (ex.:
...
export async function extrairTotaisDePlanilha(buffer: Buffer, tipo: 'xlsx' | 'csv'): Promise<TotalCandidatoPlanilha[]> {
  ...
  for (const { cabecalho, linhas } of abas) {
    for (const linha of linhas) {
      const rotuloLinha = linha.find((v): v is string => typeof v === 'string' && v.trim().length > 0)

      linha.forEach((valor, indice) => {
        if (typeof valor !== 'number') return
        const rotulo = rotuloLinha ?? cabecalho[indice] ?? `coluna ${indice + 1}`
        candidatos.push({ rotulo: rotulo.trim(), valor })
      })
    }
  }
