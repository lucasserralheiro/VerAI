...
function Diferenca({ divergencia }: { divergencia: Divergencia }) {
	...
	return (
		<td className="whitespace-nowrap px-3 py-1.5 text-right font-mono tabular-nums">
			<span className={`font-semibold ${critico ? "text-severidade-critico" : "text-severidade-maior"}`}>
...
function Linha({ divergencia }: { divergencia: Divergencia }) {
	return (
		<tr className="border-b border-line last:border-0">
			...
			<td className="px-3 py-1.5 text-navy-800">
				...
				{divergencia.na_proposta !== null && divergencia.no_aditivo !== null && (
					<div className="pt-0.5 font-mono text-[11px] text-navy-300">
						proposta {divergencia.na_proposta} + aditivo {divergencia.no_aditivo}
					</div>
				)}
			</td>
			<td className="whitespace-nowrap px-3 py-1.5 text-xs text-navy-300">
				{divergencia.unidade}
			</td>
			<td className="whitespace-nowrap px-3 py-1.5 text-right font-mono tabular-nums text-navy-800">
				{divergencia.no_contrato}
			</td>
			<td className="whitespace-nowrap px-3 py-1.5 text-right font-mono font-semibold tabular-nums text-navy-800">
		...
		<section className="mt-6" aria-labelledby="titulo-divergencia">
			...
			<div
				className={`rounded-t-md border border-b-0 p-4 ${
					critico
						? "border-severidade-critico/30 bg-severidade-critico-fundo"
						: "border-severidade-maior/30 bg-severidade-maior-fundo"
				}`}
			>
				<h2
					id="titulo-divergencia"
					className={`text-lg font-semibold ${critico ? "text-severidade-critico" : "text-severidade-maior"}`}
				>
					{titulo}
				</h2>
				...
				<p className="mt-1 max-w-3xl text-sm text-navy-800">
					{comAditivo ? (
						<>
				...
				{!comAditivo && (
					<button
						...
						className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-teal-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-teal-600"
					>
			...
			<div className="overflow-hidden rounded-b-md border border-line bg-white">
				<div
				...
				<p className="border-t border-line px-4 py-3 text-xs text-navy-300">
					{comAditivo ? (
